...
Promise.all([edgePromise, nodePromise]).then((values) => {
  ...
  select('#size').on('change', function () {
    const menu = select(this).node();
    const choice = select(menu.options[menu.selectedIndex]);
    const [mode, level] = choice.attr('data-name').split('/');
    ...
    switch(mode) {
      ...
      case 'degree':
        scene.setDegreeSize(select('#filter').node().valueAsNumber, level);
      break;
  ...
  select('#filter').on('input', function () {
    ...
    if (scene.degreeSize) {
      const menu = select('#size').node();
      const choice = select(menu.options[menu.selectedIndex]);
      const [mode, level] = choice.attr('data-name').split('/');
      scene.setDegreeSize(year, level);
    }
  });
